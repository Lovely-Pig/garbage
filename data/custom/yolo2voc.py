...
def convert_file(file_id: int, img_dir: str, input_dir: str, output_dir: str):
    print(f'{input_dir}/{file_id}.txt -> {output_dir}/{file_id}.xml')
    
    img = cv2.imread(f'{img_dir}/{file_id}.jpg')
...
def convert_files(num: int, img_dir: str, input_dir: str, output_dir: str):
    if not os.path.exists(output_dir):
        os.mkdir(output_dir)
        
    for file_id in range(1, num + 1):
        convert_file(file_id, img_dir, input_dir, output_dir)


if __name__ == '__main__':
    convert_files(num=122, img_dir='images2', input_dir='labels', output_dir='labels2')
